...
class Database{
    ...
    private function RunCMD($CMD, $Values1Arr = null, $Values2Arr = null){
        ...
        //hvis der ikke er non connection start en
        if(!$this->IsConnected){
     ...
     */
    public function Exist($Table, $ValuesCheck){
       ...
       // print_r($resultPkey);
        ...
        $result = $this->GetResult("SELECT `{$resultPkey["Column_name"]}` FROM `{$Table}` {$whereValue["nonValues"]};",$whereValue["values"]);

        return $result[$resultPkey["Column_name"]];
    }
